    ...
    const fetchData = async () => {
      ...
      try {
        const responses = await Promise.all([
          ...
          fetch(`http://localhost:4000/api/expense/monthly-summary/${year}/${month}`), 
          fetch(`http://localhost:4000/api/expense/daily-summary/${year}/${month}/${day}`),
          ...
          fetch(`http://localhost:4000/api/loan`),
          fetch(`http://localhost:4000/api/loan`),
          fetch(`http://localhost:4000/api/receiving`),
          fetch(`http://localhost:4000/api/receiving`)
        ]);
