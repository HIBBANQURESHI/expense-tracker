...
const Expense = () => {
  ...
  const fetchSales = async () => {
    try {
      const response = await axios.get("https://akc-expense-server.vercel.app/api/jahez");
  ...
  const fetchSummary = async () => {
    try {
      const response = await axios.get("https://akc-expense-server.vercel.app/api/jahez/summary/total");
  ...
  const handleDelete = async (id) => {
    try {
      await axios.delete(`https://akc-expense-server.vercel.app/api/jahez/${id}`);
  ...
  return (
    ...
    >
      <h1 className="text-3xl font-bold text-gray-800 mb-8">Jahez Delivery Management</h1>
      ...
      <div className="w-full max-w-6xl mb-6 space-y-4">
        ...
        <Link href="/CreateJahez" className="block">
              ...
              filteredSales.map((sale) => (
                <tr key={sale._id} className="border-b border-gray-700 hover:bg-gray-600 transition">
                  ...
                  <td className="px-5 py-4 text-sm">
                    <Link href={`/UpdateJahez/${sale._id}`}>
