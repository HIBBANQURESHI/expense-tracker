...
const Expense = () => {
  ...
  const fetchSales = async () => {
    try {
      const response = await axios.get("http://localhost:4000/api/marsool");
  ...
  const fetchSummary = async () => {
    try {
      const response = await axios.get("http://localhost:4000/api/marsool/summary/total");
  ...
  const handleDelete = async (id) => {
    try {
      await axios.delete(`http://localhost:4000/api/marsool/${id}`);
  ...
  return (
    ...
    >
      <h1 className="text-3xl font-bold text-gray-800 mb-8">Marsool Delivery Management</h1>
      ...
      <div className="w-full max-w-6xl mb-6 space-y-4">
        ...
        <Link href="/CreateMarsool" className="block">
