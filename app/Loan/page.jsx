...
const LoanList = () => {
  ...
  const fetchLoans = async () => {
    try {
      const response = await axios.get("https://akc-expense-server.vercel.app/api/loan");
      if (response.data) {
  ...
  const handleDelete = async (id) => {
    try {
      await axios.delete(`https://akc-expense-server.vercel.app/api/loan/${id}`);
  ...
  return (
    ...
    >
      <h1 className="text-4xl font-bold mb-6">Loans</h1>
        ...
        <table className="min-w-full leading-normal">
          <thead>
            <tr className="bg-gray-100 text-black">
              ...
              <th className="px-5 py-3 text-left text-sm font-semibold">Remaining</th>
          ...
          <tbody>
            {filteredLoans.length > 0 ? (
              ...
              <tr>
                <td colSpan="6" className="px-5 py-4 text-center text-gray-400">No loans found.</td>
              </tr>
            )}
          </tbody>
        </table>
