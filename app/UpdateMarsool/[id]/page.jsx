...
const UpdateExpense = () => {
  const [sale, setSale] = useState({ deliveries: '', amount: '', date:'' });
  const { id } = useParams(); 
  ...
  const fetchSaleDetails = async () => {
    try {
      const response = await axios.get(`https://akc-expense-server.vercel.app/api/marsool/${id}`);
      if (response.data) {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      await axios.put(`https://akc-expense-server.vercel.app/api/marsool/${id}`, sale);
